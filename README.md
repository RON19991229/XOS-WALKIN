...
| 🔁 **Banned/under-age 用户能反复刷** | 30 分钟冷却现在基于 **IC**（不是 customer_id），并且在所有判断前最先执行。任何近 30 分钟的尝试（不论 approved/banned/age）都会被拒绝，**不会**生成新的 visit 记录 |
...
> 所有改动**不影响数据库 schema**。直接重新部署即可，**不需要**跑迁移 SQL。
