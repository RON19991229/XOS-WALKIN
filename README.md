...
**Step 1:** 在 Supabase SQL Editor 运行 **`migration-v2.1-cooldown-trigger.sql`**（安装数据库 trigger，强制 30 分钟冷却）。这个脚本是幂等的，安全地可以重复运行。

**Step 2:** 把新的代码（这个 zip）部署到 Netlify。

> **数据库 trigger 是关键防护层**。如果只更新前端代码而没有跑 SQL，理论上仍然存在被绕过的可能。两步都做，才能 100% 阻止重复打卡。
