...
export default function RegisterPage() {
  ...
  const isMinor = ageCategory === '12-15';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 3) Emergency: relationship + phone (both must be filled or both empty)
    ...
    if (existingPhone) {
      ...
      if (existingPhone.status === 'banned') {
        // Phone belongs to banned customer — block and show banned screen
    ...
    // Check 2: dob from IC (Malaysian)
    let dob: string | null = null;
    if (nationality === 'malaysian') {
      const dobDate = new Date(
        2000 + parseInt(ic.slice(0, 2)) <= new Date().getFullYear()
          ? 2000 + parseInt(ic.slice(0, 2))
          : 1900 + parseInt(ic.slice(0, 2)),
        parseInt(ic.slice(2, 4)) - 1,
        parseInt(ic.slice(4, 6))
      );
    ...
    // Save customer to session, go to reminders page
    ...
    <main className="min-h-screen flex flex-col bg-ink">
      ...
      <section className="flex-1 px-5 py-8 max-w-md mx-auto w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Guardian fields - only for minors 12-15 */}
          {isMinor && (
