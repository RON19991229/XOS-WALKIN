'use client';

import { useEffect, useState } from 'react';
import { useRouter } from 'next/navigation';
import { createClient } from '@/lib/supabase-client';
import { Lang, t } from '@/lib/i18n';
import { Customer } from '@/lib/types';

export default function BannedPage() {
  const router = useRouter();
  const supabase = createClient();
  const [lang, setLang] = useState<Lang>('en');
  const [customer, setCustomer] = useState<Customer | null>(null);

  useEffect(() => {
    const savedLang = sessionStorage.getItem('xf-lang') as Lang | null;
    if (savedLang) setLang(savedLang);

    const data = sessionStorage.getItem('xf-customer');
    if (!data) {
      router.replace('/checkin');
      return;
    }

    const c: Customer = JSON.parse(data);
    setCustomer(c);

    // Log denied attempt
    supabase
      .from('visits')
      .insert({
        customer_id: c.id,
        ic: c.ic,
        status: 'denied_banned',
      })
      .then(() => {});

    // Auto-return after 20 seconds
    const timeout = setTimeout(() => {
      sessionStorage.removeItem('xf-customer');
      sessionStorage.removeItem('xf-ic');
      router.replace('/checkin');
    }, 20000);

    return () => clearTimeout(timeout);
  }, [router, supabase]);

  if (!customer) return null;

  return (
    <main className="min-h-screen bg-danger text-bone flex flex-col relative overflow-hidden animate-shake">
      <div className="h-6 stripes-red" />

      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 text-center relative z-10">
        <div className="animate-slam mb-6">
          <div className="w-32 h-32 md:w-40 md:h-40 bg-ink text-danger flex items-center justify-center text-7xl md:text-8xl font-display border-8 border-bone -rotate-3">
            ✕
          </div>
        </div>

        <p
          className="font-mono text-xs tracking-[0.4em] mb-3 animate-slam"
          style={{ animationDelay: '0.1s', animationFillMode: 'both', opacity: 0 }}
        >
          // STATUS: DENIED
        </p>

        <h1
          className="font-display text-7xl md:text-9xl leading-[0.85] tracking-tighter mb-4 animate-slam"
          style={{ animationDelay: '0.2s', animationFillMode: 'both', opacity: 0 }}
        >
          {t(lang, 'banned')}
        </h1>

        <div
          className="bg-bone text-ink px-6 py-3 mb-8 inline-block animate-slam"
          style={{ animationDelay: '0.3s', animationFillMode: 'both', opacity: 0 }}
        >
          <p className="font-display text-2xl md:text-3xl tracking-wider">
            {customer.name.toUpperCase()}
          </p>
        </div>

        <p
          className="font-display text-xl md:text-2xl tracking-wider max-w-md animate-slam mb-3"
          style={{ animationDelay: '0.4s', animationFillMode: 'both', opacity: 0 }}
        >
          {t(lang, 'bannedSub')}
        </p>

        <p
          className="font-display text-lg md:text-xl tracking-wider max-w-md animate-slam mb-8 opacity-90"
          style={{ animationDelay: '0.5s', animationFillMode: 'both', opacity: 0 }}
        >
          {t(lang, 'bannedContact')}
        </p>

        <div className="font-mono text-xs opacity-70 mt-4">
          REF: {customer.ic} · {new Date().toISOString().split('T')[0]}
        </div>
      </div>

      <div className="h-6 stripes-red" />
    </main>
  );
}
