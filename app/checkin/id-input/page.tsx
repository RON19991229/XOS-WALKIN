...
export default function IdInputPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Existing customer: check 30-minute cooldown
    if (customer) {
      ...
      if (recentVisits && recentVisits.length > 0) {
        const lastVisit = new Date(recentVisits[0].visited_at);
        const now = new Date();
        ...
        if (minutesSince < 30) {
          ...
          setError(
            lang === 'zh'
              ? `您 ${Math.floor(minutesSince)} 分钟前刚入场过。请等候 ${remaining} 分钟后再试。`
              : lang === 'ms'
              ? `Anda baru sahaja daftar masuk ${Math.floor(minutesSince)} minit yang lalu. Sila tunggu ${remaining} minit lagi.`
              : `You just checked in ${Math.floor(minutesSince)} minute(s) ago. Please wait ${remaining} more minutes before trying again.`
          );
          return;
        }
      }
